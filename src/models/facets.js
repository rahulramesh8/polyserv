...
model.getAccessToken = function(bearerToken, callback) {
  pg.connect(connString, (err, client, done) => {
    ...
    client.query(
      "SELECT access_token, client_id, expires, user_id FROM oauth_access_tokens " +
...
model.getClient = function(clientId, clientSecret, callback) {
  pg.connect(connString, (err, client, done) => {
    ...
    client.query(
      "SELECT client_id, client_secret, redirect_uri FROM oauth_clients WHERE " +
        "client_id = $1",
      [clientId],
      (err, result) => {
        if (err || !result.rowCount) return callback(err);
...
model.getRefreshToken = function(bearerToken, callback) {
  pg.connect(connString, (err, client, done) => {
    if (err) return callback(err);
    client.query(
      "SELECT refresh_token, client_id, expires, user_id FROM oauth_refresh_tokens " +
...
model.saveAccessToken = function(
  ...
  pg.connect(connString, (err, client, done) => {
    if (err) return callback(err);
    client.query(
      "INSERT INTO oauth_access_tokens(access_token, client_id, user_id, expires) " +
        "VALUES ($1, $2, $3, $4)",
      [accessToken, clientId, userId, expires],
      (err, result) => {
        callback(err);
        done();
      }
